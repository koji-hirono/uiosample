...
func (s *Server) procN3ARP(eth *znet.EtherHdr, payload []byte) error {
	return procARP(s.port1.driver, eth, payload)
}

func (s *Server) procN3ICMP(eth *znet.EtherHdr, ip *znet.IPv4Hdr, payload []byte) error {
	return procICMP(s.port1.driver, eth, ip, payload)
}
...
func (s *Server) resolveMAC(ip []byte) []byte {
	switch ip[0] {
	case 60:
		return []byte{0x08, 0x00, 0x27, 0xa0, 0xa9, 0x3e}
	case 70:
		return []byte{0x08, 0x00, 0x27, 0xdc, 0x7d, 0x2a}
	default:
